...
app.use(
  "/api/chat/files",
  express.static(path.join(__dirname, "public", "chat_files"))
);
    ...
    bb.on("close", async () => {
      try {
        ...
        const uniqueFileName =
          fileUrl.split("/").pop() || path.basename(fileSavePath);
...
io.use(async (socket, next) => {
  try {
    ...
    try {
      ...
      console.log(
        `Verifying token (starts with): ${token.substring(0, 15)}...`
      );

      // IMPORTANT: Using process.env.KEY which matches the token generation in User.genrateauth
      console.log(
        `Using KEY env variable: ${process.env.KEY ? "exists" : "undefined"}`
      );
      ...
      if (!JWT_KEY) {
        ...
        return next(
          new Error("Server configuration error: JWT key not defined")
        );
      }
      ...
      return next(
        new Error(`Authentication error: Invalid token (${jwtError.message})`)
      );
    }
  ...
  try {
    ...
    socket.on("sendMessage", async (data) => {
      try {
        ...
        if (!recipientId) {
          console.error(
            `Recipient not found in conversation: ${conversationId}`
          );
        ...
        console.log(
          `Message sent from ${socket.user.username} to conversation ${conversationId}`
        );
      } catch (error) {
    ...
    socket.on("markAsRead", async ({ conversationId }) => {
      try {
        ...
        if (!conversation) {
          console.error(
            `Conversation not found for markAsRead: ${conversationId}`
          );
          socket.emit("error", { message: "Conversation not found" });
          return;
        }
        ...
        if (conversation.unreadBy.includes(socket.user._id)) {
          ...
          if (otherParticipantId) {
            ...
            console.log(
              `Conversation ${conversationId} marked as read by ${socket.user.username}`
            );
          }
        }
      } catch (error) {
    ...
    socket.on("sendFileMessage", async (data) => {
      try {
        ...
        console.log(
          `File message sent from ${socket.user.username} to conversation ${conversationId}`
        );
      } catch (error) {
...
async function broadcastUserStatus(userId, isOnline) {
  try {
    ...
    console.log(
      ...
      } status for user ${userId} to ${otherParticipants.size} users`
    );
  } catch (error) {
