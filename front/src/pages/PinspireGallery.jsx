...
const PinspireGallery = () => {
  ...
  const handleImageDownload = async (imageUrl, imageTitle) => {
    try {
      // Create a temporary anchor element
      const link = document.createElement('a');
      
      // Set the download attribute with the image title (sanitized for filename)
      const filename = imageTitle.replace(/[^a-z0-9]/gi, '_').toLowerCase() + '.jpg';
      ...
      trackImageDownload(selectedImage._id);
    } catch (error) {
  ...
  const trackImageDownload = async (imageId) => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        console.log(`Image downloaded. Total downloads: ${data.totalDownloads}`);
        
        // Update the download count in the selected image if needed
        if (selectedImage && selectedImage._id === imageId) {
          setSelectedImage(prev => ({
            ...prev,
            downloads: {
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <motion.button 
                        ...
                        onClick={() => handleImageDownload(selectedImage.imageUrl, selectedImage.title)}
                        whileHover={{ scale: 1.1 }}
                        whileTap={{ scale: 0.9 }}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center text-base-content/70 text-sm">
                        ...
                        {selectedImage.downloads ? (
                          <span>{selectedImage.downloads.total?.toLocaleString() || 0}</span>
                        ) : (
                          <span>0</span>
                        )}
                      </div>
